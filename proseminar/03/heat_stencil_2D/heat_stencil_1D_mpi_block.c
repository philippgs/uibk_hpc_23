...
int main(int argc, char** argv) {
	...
	for(int t = 0; t < T; t++) {
		...
		if(myRank > 0) {
			if(myRank % 2 == 0) {
				MPI_Recv(&B[0], 1, MPI_DOUBLE, myRank - 1, 0, MPI_COMM_WORLD, &(status[1]));
				MPI_Send(&A[1], 1, MPI_DOUBLE, myRank - 1, 0, MPI_COMM_WORLD);
			} else {
				MPI_Send(&A[1], 1, MPI_DOUBLE, myRank - 1, 0, MPI_COMM_WORLD);
				MPI_Recv(&B[0], 1, MPI_DOUBLE, myRank - 1, 0, MPI_COMM_WORLD, &(status[1]));
			}
		...
		if(myRank < numProcs - 1) {
			if(myRank % 2 == 0) {
				MPI_Recv(&B[rankLength + 1], 1, MPI_DOUBLE, myRank + 1, 0, MPI_COMM_WORLD,
				         &(status[2]));
				MPI_Send(&A[rankLength], 1, MPI_DOUBLE, myRank + 1, 0, MPI_COMM_WORLD);
			} else {
				MPI_Send(&A[rankLength], 1, MPI_DOUBLE, myRank + 1, 0, MPI_COMM_WORLD);
				MPI_Recv(&B[rankLength + 1], 1, MPI_DOUBLE, myRank + 1, 0, MPI_COMM_WORLD,
				         &(status[2]));
