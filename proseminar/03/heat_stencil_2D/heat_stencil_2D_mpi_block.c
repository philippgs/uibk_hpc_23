...
int main(int argc, char** argv) {
	...
	for(int t = 0; t < T; t++) {
		...
		if(myRank % 2 == 0) {
			MPI_Send(&A[INDEX(1, 1)] /* north */, 1, topBottom, top, 0, MPI_COMM_WORLD);
			MPI_Recv(&A[INDEX(1, 0)] /* north */, 1, topBottom, top, 0, MPI_COMM_WORLD, &(reqs[0]));
		} else {
			MPI_Recv(&A[INDEX(1, 0)] /* north */, 1, topBottom, top, 0, MPI_COMM_WORLD, &(reqs[0]));
			MPI_Send(&A[INDEX(1, 1)] /* north */, 1, topBottom, top, 0, MPI_COMM_WORLD);
		}
		if(myRank % 2 == 0) {
			MPI_Recv(&A[INDEX(1, rankLengthY + 1)] /* south */, 1, topBottom, bottom, 0,
			         MPI_COMM_WORLD, &(reqs[1]));
			MPI_Send(&A[INDEX(1, rankLengthY)] /* south */, 1, topBottom, bottom, 0,
			         MPI_COMM_WORLD);
		} else {
			MPI_Send(&A[INDEX(1, rankLengthY)] /* south */, 1, topBottom, bottom, 0,
			         MPI_COMM_WORLD);
		...
		if(myRank % 2 == 0) {
			MPI_Recv(&A[INDEX(rankLengthX + 1, 1)] /* west */, 1, leftRight, right, 0,
			         MPI_COMM_WORLD, &(reqs[2]));
			MPI_Send(&A[INDEX(1, 1)] /* west */, 1, leftRight, left, 0, MPI_COMM_WORLD);
		} else {
			MPI_Send(&A[INDEX(1, 1)] /* west */, 1, leftRight, left, 0, MPI_COMM_WORLD);
			MPI_Recv(&A[INDEX(rankLengthX + 1, 1)] /* west */, 1, leftRight, right, 0,
			         MPI_COMM_WORLD, &(reqs[2]));
		...
		if(myRank % 2 == 0) {
			MPI_Recv(&A[INDEX(0, 1)] /* east */, 1, leftRight, left, 0, MPI_COMM_WORLD, &(reqs[3]));
			MPI_Send(&A[INDEX(rankLengthX, 1)] /* east */, 1, leftRight, right, 0, MPI_COMM_WORLD);
		} else {
			MPI_Send(&A[INDEX(rankLengthX, 1)] /* east */, 1, leftRight, right, 0, MPI_COMM_WORLD);
			MPI_Recv(&A[INDEX(0, 1)] /* east */, 1, leftRight, left, 0, MPI_COMM_WORLD, &(reqs[3]));
		}
